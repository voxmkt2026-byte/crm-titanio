...
function initializeCopilot(){
 ...
 async function api(path,data=null){
  const timeout=path==='iniciar'?40000:['segmentos','finalizar'].includes(path)?45000:22000;
 ...
 async function flush(current){
  ...
  if(current.stopping&&current.batch?.retry_analysis)current.batch=null;
  ...
  if(!current.batch&&!current.stopping&&current.analysisPending&&current.quietRetries<2&&Date.now()>=current.retryAfter){
   current.batch={batch_id:crypto.randomUUID(),segments:[],retry_analysis:true};current.quietRetries++;
  }
  ...
  current.pending=api('segmentos',{session_id:current.session,...batch}).then(data=>{
   ...
   // Reset only when a spoken batch is delivered, not while an older retry is in flight.
   if(!batch.retry_analysis)current.quietRetries=0;
   current.analysisPending=data.analysis_pending===true;
   current.retryAfter=Date.now()+current.updateMs;
 ...
 async function begin(){
  ...
  const current={attemptId,session:null,audio:null,queue:[],batch:null,pending:null,analysisPending:false,quietRetries:0,retryAfter:0,updateMs:10000,stopping:false,incomplete:false,partials:new Set(),timer:null,expiry:null,boot:null};run=current;controls();
  current.boot=(async()=>{
   ...
   current.updateMs=Math.max(5,Number(result.update_seconds||settings.update_seconds)||10)*1000;

...
final class CallSyncService {
 ...
 public function __construct(private PDO $db,private IntegrationConfig $config,private string $account='api4com'){$this->account=CallAccounts::key($account);$this->config=CallAccounts::forAccount($config,$account);$this->matcher=new CallLeadMatcher($db,$account);}
 ...
 public function run(int $pages=3,int $jobs=1):array{$synced=0;try{$this->db->exec("INSERT INTO call_sync_state(provider,status,locked_at) VALUES('{$this->account}','running',NOW()) ON DUPLICATE KEY UPDATE status='running',locked_at=NOW(),last_error_code=NULL");for($page=1;$page<=max(1,min(100,$pages));$page++){ $calls=$this->fetchCalls($page);if(!$calls)break;foreach($calls as$call)if(!empty($call['record_url'])){$this->upsert($call);$synced++;}}$processed=$jobs>0?$this->process($jobs):0;$result=compact('synced','processed');$last=$this->db->query("SELECT MAX(started_at) FROM call_records WHERE provider='{$this->account}'")->fetchColumn();$q=$this->db->prepare("UPDATE call_sync_state SET status='idle',last_started_at=:last,last_success_at=NOW(),last_counts=:counts,last_error_code=NULL,locked_at=NULL WHERE provider='{$this->account}'");$q->execute([':last'=>$last?:null,':counts'=>json_encode($result,JSON_UNESCAPED_UNICODE)]);return$result;}catch(Throwable$e){$code=substr(preg_replace('/[^A-Z0-9_]/','',strtoupper($e->getMessage()))?:'UNKNOWN_ERROR',0,80);try{$this->db->prepare("INSERT INTO call_sync_state(provider,status,last_error_code) VALUES('{$this->account}','failed',:error) ON DUPLICATE KEY UPDATE status='failed',last_error_code=VALUES(last_error_code),locked_at=NULL")->execute([':error'=>$code]);}catch(Throwable){}throw$e;}}
 public function syncPage(int $page):array{$page=max(1,min(100,$page));$calls=$this->fetchCalls($page);$synced=0;foreach($calls as$call)if(!empty($call['record_url'])){$this->upsert($call);$synced++;}$result=['page'=>$page,'source_count'=>count($calls),'synced'=>$synced,'has_more'=>count($calls)>=20];$last=$this->db->query("SELECT MAX(started_at) FROM call_records WHERE provider='{$this->account}'")->fetchColumn();$q=$this->db->prepare("INSERT INTO call_sync_state(provider,last_started_at,last_success_at,status,last_counts,last_error_code,locked_at) VALUES('{$this->account}',:last,NOW(),'idle',:counts,NULL,NULL) ON DUPLICATE KEY UPDATE last_started_at=VALUES(last_started_at),last_success_at=NOW(),status='idle',last_counts=VALUES(last_counts),last_error_code=NULL,locked_at=NULL");$q->execute([':last'=>$last?:null,':counts'=>json_encode($result,JSON_UNESCAPED_UNICODE)]);return$result;}
 public function importLegacy(string $directory,int $pages=100,bool $dryRun=true):array{if($this->account!=='api4com')throw new RuntimeException('LEGACY_ACCOUNT_UNSUPPORTED');$result=['with_recording'=>0,'with_analysis'=>0,'imported'=>0,'voicemail'=>0,'invalid'=>0];for($page=1;$page<=max(1,$pages);$page++){foreach($this->fetchCalls($page)as$c){if(empty($c['record_url']))continue;$result['with_recording']++;$x=(string)($c['id']??'');$file=rtrim($directory,'/\\').DIRECTORY_SEPARATOR.hash('sha256',$x).'.json';if(!is_file($file))continue;$result['with_analysis']++;try{$raw=file_get_contents($file);$a=json_decode((string)$raw,true,512,JSON_THROW_ON_ERROR);$transcript=mb_strtolower((string)($a['transcript']??''));$a['call_outcome']=(str_contains($transcript,'caixa postal')||str_contains($transcript,'correio de voz'))?'voicemail':($a['call_outcome']??'conversation');$a=(new AnalysisNormalizer())->normalize($a,'gemini',(string)($a['provider_model']??'legado'));if($a['call_outcome']==='voicemail'){$result['voicemail']++;if(!$dryRun){$this->upsert($c);$this->db->prepare("UPDATE call_records SET outcome=IF(link_status='manual' AND outcome='invalid_number',outcome,'voicemail'),analysis_status='discarded',recording_status='discarded' WHERE provider='{$this->account}' AND external_id=:x")->execute([':x'=>$x]);}continue;}if(!$dryRun){$this->upsert($c);$id=(int)$this->db->query("SELECT id FROM call_records WHERE provider='{$this->account}' AND external_id=".$this->db->quote($x))->fetchColumn();$j=json_encode($a,JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES|JSON_THROW_ON_ERROR);$q=$this->db->prepare("INSERT INTO call_analyses(call_id,analysis_version,summary,transcript,sentiment,lead_temperature,sales_stage,call_outcome,overall_score,provider,provider_model,payload,analyzed_at) VALUES(:id,:v,:s,:t,:se,:lt,:st,:o,:sc,:p,:m,:j,NOW()) ON DUPLICATE KEY UPDATE payload=VALUES(payload),summary=VALUES(summary),transcript=VALUES(transcript)");$q->execute([':id'=>$id,':v'=>$a['analysis_version'],':s'=>$a['summary'],':t'=>$a['transcript'],':se'=>$a['sentiment'],':lt'=>$a['lead_temperature'],':st'=>$a['sales_stage'],':o'=>$a['call_outcome'],':sc'=>$a['scores']['overall'],':p'=>$a['provider'],':m'=>$a['provider_model'],':j'=>$j]);$this->db->prepare("UPDATE call_records SET outcome=IF(link_status='manual' AND outcome='invalid_number',outcome,:o),analysis_status='completed' WHERE id=:id")->execute([':o'=>$a['call_outcome'],':id'=>$id]);$this->db->prepare("UPDATE call_processing_jobs SET status='completed',completed_at=NOW() WHERE call_id=:id AND job_type='analyze'")->execute([':id'=>$id]);$this->history($id);}$result['imported']++;}catch(Throwable$e){$result['invalid']++;}}}return$result;}
 public function importCachedAnalysis(int $callId, string $directory): array
 {
     if ($this->account!=='api4com') throw new RuntimeException('LEGACY_ACCOUNT_UNSUPPORTED');
     $q=$this->db->prepare("SELECT external_id FROM call_records WHERE id=:id AND provider='{$this->account}' LIMIT 1");
 ...
 public function saveAnalysis(int $callId, array $a): void
 {
     ...
     try {
         if ($a['call_outcome']==='voicemail') {
             ...
             $this->db->prepare("UPDATE call_records SET outcome=IF(link_status='manual' AND outcome='invalid_number',outcome,'voicemail'),analysis_status='discarded',recording_status='discarded',recording_path=NULL,recording_mime=NULL,recording_size=NULL,recording_checksum=NULL,recording_expires_at=NULL,last_error_code=NULL WHERE id=:id")->execute([':id'=>$callId]);
         } else {
             ...
             $this->db->prepare("UPDATE call_records SET outcome=IF(link_status='manual' AND outcome='invalid_number',outcome,:o),analysis_status='completed',last_error_code=NULL WHERE id=:id")->execute([':o'=>$a['call_outcome'],':id'=>$callId]);
             $this->history($callId);
         }
 ...
 private function upsert(array $c):void{$x=trim((string)($c['id']??''));if($x===''||!preg_match('/^[A-Za-z0-9._:-]{1,160}$/',$x))return;$match=$this->matcher->match($c);$email=strtolower(trim((string)($c['email']??'')));$q=$this->db->prepare("INSERT INTO call_records(provider,external_id,lead_id,user_id,agent_external_key,contact_phone,normalized_phone,direction,started_at,ended_at,duration,hangup_cause,call_price,link_status,recording_url_hash) VALUES('{$this->account}',:x,:l,:u,:a,:p,:n,:direction,:s,:e,:du,:h,:price,:link,:rh) ON DUPLICATE KEY UPDATE lead_id=IF(link_status='manual',lead_id,VALUES(lead_id)),user_id=IF(link_status='manual',user_id,COALESCE(VALUES(user_id),user_id)),contact_phone=VALUES(contact_phone),normalized_phone=VALUES(normalized_phone),direction=VALUES(direction),link_status=IF(link_status='manual',link_status,VALUES(link_status)),updated_at=NOW()");$q->execute([':x'=>$x,':l'=>$match['lead_id'],':u'=>$match['user_id'],':a'=>$email?:null,':p'=>$match['contact_phone'],':n'=>$match['normalized_phone'],':direction'=>$match['direction'],':s'=>$this->date($c['started_at']??null),':e'=>$this->date($c['ended_at']??null),':du'=>max(0,(int)($c['duration']??0)),':h'=>mb_substr((string)($c['hangup_cause']??''),0,120),':price'=>is_numeric($c['call_price']??null)?$c['call_price']:null,':link'=>$match['link_status'],':rh'=>hash('sha256',(string)$c['record_url'])]);$id=(int)$this->db->query("SELECT id FROM call_records WHERE provider='{$this->account}' AND external_id=".$this->db->quote($x))->fetchColumn();$this->db->prepare("INSERT IGNORE INTO call_processing_jobs(call_id,job_type,status) VALUES(:id,'analyze','pending')")->execute([':id'=>$id]);(new PhoneReconciler($this->db))->reconcile($this->account,$id,$c);$this->history($id);}
 ...
 {
     $rows=$this->db->query("SELECT c.id FROM call_processing_jobs j JOIN call_records c ON c.id=j.call_id WHERE c.provider='{$this->account}' AND j.job_type='analyze' AND ((j.status IN('pending','failed') AND j.available_at<=NOW()) OR (j.status='processing' AND j.locked_at<DATE_SUB(NOW(),INTERVAL 10 MINUTE))) ORDER BY j.id LIMIT ".max(1,min(10,$limit)))->fetchAll();
     $done=0;

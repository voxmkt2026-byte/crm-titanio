...
final class CurlHttpClient implements HttpClientInterface
{
    ...
    public function download(string $url, array $headers, int $maxBytes): DownloadedFile
    {
        ...
        curl_setopt($handle, CURLOPT_WRITEFUNCTION, function ($curl, string $chunk) use ($stream, $maxBytes, &$size, &$tooLarge): int {
            $length = strlen($chunk);
        ...
        if ($tooLarge) {
            @unlink($path);
            throw new AppException('A gravação excede o limite permitido.', 'AUDIO_TOO_LARGE', 413);
        }
        ...
        return new DownloadedFile($path, $mime, $size);
    }

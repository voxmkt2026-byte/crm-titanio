...
final class CopilotController extends Controller {
 ...
 public function segments():void{$this->respond(fn($p)=>$this->service()->segments($this->actor,$this->all(),is_string($p['session_id']??null)?$p['session_id']:'',is_string($p['batch_id']??null)?$p['batch_id']:'',is_array($p['segments']??null)?$p['segments']:[],($p['retry_analysis']??false)===true),true);}
